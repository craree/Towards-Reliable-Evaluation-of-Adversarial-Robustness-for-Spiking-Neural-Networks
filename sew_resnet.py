...
__all__ = ['SEWResNet', 'SEWResNetCifar', 'sew_resnet18', 'sew_resnet34', 'sew_resnet50', 'sew_resnet101',
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.node1 = node(channels=planes)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = norm_layer(planes)
        self.node2 = node(channels=planes)
        self.downsample = downsample
        if downsample is not None:
            self.downsample_sn = node(channels=planes * self.expansion)
        self.stride = stride
        self.cnf = cnf
...
class SEWResNet(BaseModule):
    def __init__(self, block, layers, num_classes=1000, step=8, encode_type="direct", zero_init_residual=False,
        ...
        encode = kwargs["encoder"] if "encoder" in kwargs else 'dir'
        if encode == 'poi':
        ...
        self.node1 = self.node(channel=self.inplane)
        self.avgpool0 = nn.AvgPool2d(kernel_size=3, stride=2, padding=1)
...
class SEWResNetCifar(BaseModule):
    def __init__(self, block, layers, num_classes=1000, step=8, encode_type="direct", zero_init_residual=False,
        ...
        self.conv1 = nn.Conv2d(init_channel, self.inplanes, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = norm_layer(self.inplanes)
        self.node1 = self.node(channels=self.inplanes)
        self.avgpool1 = nn.AvgPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 128, layers[0], cnf=cnf, node=self.node, **kwargs)
        # self.layer1 = self._make_layer(block, 128, layers[0], stride=2, cnf=cnf, node=self.node, **kwargs)
        self.layer2 = self._make_layer(block, 256, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], cnf=cnf, node=self.node, **kwargs)
